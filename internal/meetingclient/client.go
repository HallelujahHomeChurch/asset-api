...
func (c *Client) ListSyncWindows(ctx context.Context, from, to time.Time) ([]Window, error) {
	...
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, c.baseURL+"/priv/operations/sync-windows?"+query.Encode(), nil)
	if err != nil {
		return nil, err
	}
	token, err := c.token(ctx)
	if err != nil {
		return nil, fmt.Errorf("authorize operations sync windows: %w", err)
	}
	request.Header.Set("Authorization", "Bearer "+token)
	response, err := c.http.Do(request)
	if err != nil {
		return nil, fmt.Errorf("list operations sync windows: %w", err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("list operations sync windows: status %d", response.StatusCode)
	}
	var envelope struct {
		Data []Window `json:"data"`
	}
	if err := json.NewDecoder(response.Body).Decode(&envelope); err != nil {
		return nil, fmt.Errorf("decode operations sync windows: %w", err)
	}
