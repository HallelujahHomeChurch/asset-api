...
type queueReceiverClient interface {
...
type AzureQueue struct {
	source queueReceiverClient
	poison queueReceiverClient
}
...
func (q *AzureQueue) Receive(ctx context.Context, visibility time.Duration) (Message, bool, error) {
	...
	if err != nil {
		return Message{}, false, fmt.Errorf("dequeue scan request: %w", err)
	}
	...
	if value == nil || value.MessageID == nil || value.PopReceipt == nil || value.MessageText == nil || value.DequeueCount == nil {
		return Message{}, false, errorsNewMalformedQueueResponse
	}
...
var errorsNewMalformedQueueResponse = errors.New("malformed Azure Queue response")
...
func (q *AzureQueue) ForwardPoison(ctx context.Context, message Message, event *Event, reason string, now time.Time) error {
	...
	record := PoisonRecord{
		Type: "asset.scan.poisoned.v1", Version: 1,
		PoisonID: message.ID + ":" + reason, Reason: reason,
		BodySHA256: hex.EncodeToString(digest[:]), DequeueCount: message.DequeueCount,
		PoisonedAt: now.UTC().Format(time.RFC3339Nano), Event: event,
	}
